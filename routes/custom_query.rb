        ...
        ConstData.maparea[maparea]["maps"].each do |map|
          ConstData.map[map]["cells"].each_key do |map_cell|
            ...
            cells.each_key.map(&:to_i).each do |cell|
              table = Sinatra::DropModelHelper.get_model(map, cell, rank, 0)
            ...
            if query_result[:count] > 0
              list.push((map / 10).to_s + "-" + (map % 10).to_s + map_cell + "&nbsp;&nbsp;&nbsp;&nbsp;" + query_result[:count].to_s + ' <span style="color:grey">/ ' + query_result[:totalCount].to_s + " = </span>" + (query_result[:count] * 100.0 / query_result[:totalCount]).round(3).to_s + '%')
            end
          end
        end
      end
